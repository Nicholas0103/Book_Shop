
const subgenre = {
    cozy: "Cozy Mystery",
    dark: "Bloody Mystery",
    supernatural: "Supernatural Mystery",
    classic: "Classic Detective",
    kids: "YA & Kid Mystery",
    historical: "Historical Mystery"
};

const BooksData = [
    {
        id: 1,
        title: "Detection Unlimited",
        author: "Supernatural Mystery", 
        cover: "/assets/book/book_1.jpg", 
        rating: 3.8,
        subgenre: subgenre.dark,
        published_year: "1971",
        price: "$8.39",
        description: "Only Hemingway can uncover which of the ten has turned hatred into murder, but has his work cut out for him.",
    },
    {
        id: 2,
        title: "The Mysterious Island",
        author: "Jules Verne", 
        cover: "/assets/book/book_2.jpg", 
        rating: 3.9,
        subgenre: subgenre.cozy,
        published_year: "1977",
        price: "9.99",
        description: "The Mysterious Island tells the tale of five Americans who, in an attempt to escape the Civil War, pilot a hot-air balloon and find themselves crashed on a deserted island somewhere in the Pacific.",
    },
    {
        id: 3,
        title: "Sherlock Holmes",
        author: "Conan Doyle", 
        cover: "/assets/book/book_3.jpg", 
        rating: 4.8,
        subgenre: subgenre.classic,
        published_year: "1924",
        price: "$15.99",
        description: "Sherlock Holme EN ROUGE Un homme est trouvé mort dans une maison inhabitée, au coeur d'un des plus sinistres quartiers de Londres. Autour de lui, des traces de sang, bien que le cadavre n'ait aucune blessure. De quoi laisser perplexes Lestrade et Gregson, les limiers de Scotland Yard. Parue en 1887, cette Étude en rouge est la premiere des enquétes de Sherlock Holmes. Nous y faisons la connaissance de l'extraordinaire détective å travers les yeux du bon Dr Watson. Nous y apprenons le « raisonnement analytique » et l'art de faire parler les indices. Ce classique du roman policier est aussi un roman d'aventures qui nous conduit dans le Nevada des Mormons et de la ruée vers l'or, oil s'enracine le mystere...",
    },
    {
        id: 4,
        title: "The Mystery of the Blue Train",
        author: "Agatha Christie", 
        cover: "/assets/book/book_4.png", 
        rating: 4.1,
        subgenre: subgenre.historical,
        published_year: "2024",
        price: "$7.50",
        description: "",
    },
    {
        id: 5,
        title: "Murder in Three Acts",
        author: "Agatha Christie", 
        cover: "/assets/book/book_5.png", 
        rating: 3.7,
        subgenre: subgenre.dark,
        published_year: "1984",
        price: "$9.40",
        description: "",
    },
    {
        id: 6,
        title: "The Mysterious Cargo",
        author: "Galila Ben-Uri", 
        cover: "/assets/book/book_6.png", 
        rating: 3.2,
        subgenre: subgenre.cozy,
        published_year: "1998",
        price: "$7.60",
        description: "",
    },
    {
        id: 7,
        title: "The Mystery of the Green Ghost",
        author: "Robert Arthur", 
        cover: "/assets/book/book_7.png", 
        rating: 3.8,
        subgenre: subgenre.supernatural,
        published_year: "1980",
        price: "$11.00",
        description: "",
    },
    {
        id: 8,
        title: "Emil and the Detectives",
        author: "Erich Kästner", 
        cover: "/assets/book/book_8.png", 
        rating: 4.4,
        subgenre: subgenre.kids,
        published_year: "1930",
        price: "$5.30",
        description: "",
    },
    {
        id: 9,
        title: "Murder Most Unladylike",
        author: "Robin Stevens", 
        cover: "/assets/book/book_9.png", 
        rating: 4.2,
        subgenre: subgenre.classic,
        published_year: "2016",
        price: "$19.55",
        description: "",
    },
    {
        id: 10,
        title: "Mercy Watson Is Missing!",
        author: "Kate DiCamillo", 
        cover: "/assets/book/book_10.png", 
        rating: 4.0,
        subgenre: subgenre.kids,
        published_year: "2023",
        price: "$3.65",
        description: "",
    },
    {
        id: 11,
        title: "New Detective",
        author: "Peter Steiner", 
        cover: "/assets/book/book_11.png", 
        rating: 2.8,
        subgenre: subgenre.classic,
        published_year: "2023",
        price: "$19.05",
        description: "",
    },
    {
        id: 12,
        title: "Girl in the White Van",
        author: "April Henry", 
        cover: "/assets/book/book_12.png", 
        rating: 4.8,
        subgenre: subgenre.dark,
        published_year: "2021",
        price: "$6.50",
        description: "",
    },
    {
        id: 13,
        title: "The Green Mile",
        author: "Stephen King", 
        cover: "/assets/book/book_13.png", 
        rating: 4.3,
        subgenre: subgenre.supernatural,
        published_year: "1999",
        price: "$20.66",
        description: "",
    },
    {
        id: 14,
        title: "The Sign of the Four",
        author: "Arthur Conan Doyle", 
        cover: "/assets/book/book_14.png", 
        rating: 4.3,
        subgenre: subgenre.historical,
        published_year: "2018",
        price: "$2.55",
        description: "",
    },
    {
        id: 15,
        title: "A Study in Scarlet",
        author: "Arthur Conan Doyle", 
        cover: "/assets/book/book_15.png", 
        rating: 4.0,
        subgenre: subgenre.historical,
        published_year: "2018",
        price: "$3.10",
        description: "",
    },
    {
        id: 16,
        title: "The Case of the Missing Marquess",
        author: "Nancy Springer", 
        cover: "/assets/book/book_16.png", 
        rating: 4.4,
        subgenre: subgenre.kids,
        published_year: "2006",
        price: "$7.30",
        description: "",
    },
    {
        id: 17,
        title: "The Prom Queen",
        author: "Robert Lawrence Stine", 
        cover: "/assets/book/book_17.png", 
        rating: 4.3,
        subgenre: subgenre.supernatural,
        published_year: "1992",
        price: "$5.96",
        description: "",
    },
    {
        id: 18,
        title: "Poached",
        author: "Stuart Gibbs", 
        cover: "/assets/book/book_18.png", 
        rating: 5.0,
        subgenre: subgenre.cozy,
        published_year: "2014",
        price: "$5.35",
        description: "",
    }
]
